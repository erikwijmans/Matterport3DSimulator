...
class SimStatePython {
  public:
    SimStatePython(SimStatePtr state, bool renderingEnabled)
        ...
        if (renderingEnabled) {
            ...
            rgb = matToNumpyArray(3, colorShape, NPY_UBYTE,
                                  (void *)state->rgb.data);
        }
        scanId = state->scanId;
  ...
  private:
    py::object matToNumpyArray(int dims, npy_intp *shape, int type,
                               void *data) {
        ...
        py::object numpyArray =
            py::reinterpret_steal<py::object>(numpyArrayHandle);
...
PYBIND11_MODULE(MatterSim, m) {
    ...
    py::class_<SimStatePython>(m, "SimState")
        ...
        .def_readonly("navigableLocations",
                      &SimStatePython::navigableLocations);
